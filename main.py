...
st.subheader('Prediction for Overall Score')

st.write('Your overall player rating prediction is {}'.format(prediction[0].round(0)))
...
feature_importance = pd.DataFrame(pd.Series(load_clf.feature_importances_, df.columns).nlargest(5))
...
st.subheader('The top 5 most important attributes to a players overall score')
